...
insert into tasks (project_id, milestone_id, title, status, position, source)
select (select id from projects where slug='home-cooked'), (select id from milestones where slug='m-home-cooked'), t.title, t.status, t.position, '{"label":"Seed data","type":"import"}'::jsonb
from (values
  ('Design sharing permission model',          'completed',   1),
  ('Sharing data model & migrations',          'completed',   2),
  ('Build invite workflow',                    'completed',   3),
  ('Invite acceptance flow',                   'completed',   4),
  ('Generate & store share links',             'completed',   5),
  ('Shared cookbook read access',              'completed',   6),
  ('Permission roles (owner / editor / viewer)','completed',  7),
  ('Sharing settings UI',                      'completed',   8),
  ('Activity feed for shared edits',           'completed',   9),
  ('Revoke & expire share links',              'completed',   10),
  ('Email acceptance flow',                    'todo',        11),
  ('Permission edge-case tests',               'in_progress', 12)
) as t(title, status, position);

-- WardrobeHarmony — Closet Import
insert into tasks (project_id, milestone_id, title, status, position, source)
select (select id from projects where slug='wardrobe-harmony'), (select id from milestones where slug='m-wardrobe-harmony'), t.title, t.status, t.position, '{"label":"Seed data","type":"import"}'::jsonb
from (values
  ('CSV import parser',                'completed',   1),
  ('Photo upload pipeline',            'completed',   2),
  ('Color extraction from photos',     'completed',   3),
  ('Map items to wardrobe schema',     'completed',   4),
  ('Dedupe imported items',            'completed',   5),
  ('Colorblind-safe palette tagging',  'in_progress', 6),
  ('Handle unsupported file formats',  'todo',        7),
  ('Bulk-edit imported items',         'todo',        8),
  ('Import progress UI',               'todo',        9),
  ('Import error reporting',           'todo',        10)
) as t(title, status, position);

-- PersonalTrainer — Client Scheduling
insert into tasks (project_id, milestone_id, title, status, position, source)
select (select id from projects where slug='personal-trainer'), (select id from milestones where slug='m-personal-trainer'), t.title, t.status, t.position, '{"label":"Seed data","type":"import"}'::jsonb
from (values
  ('Define scheduling data model',       'completed',   1),
  ('Trainer availability calendar',      'completed',   2),
  ('Client booking flow',                'in_progress', 3),
  ('Recurring sessions',                 'todo',        4),
  ('Timezone handling',                  'todo',        5),
  ('Cancellation & reschedule rules',    'todo',        6),
  ('Session reminders & notifications',  'todo',        7),
  ('Calendar sync research',             'todo',        8)
) as t(title, status, position);

-- Cascade Lounge — Spring Content Drop
insert into tasks (project_id, milestone_id, title, status, position, source)
select (select id from projects where slug='cascade-lounge'), (select id from milestones where slug='m-cascade-lounge'), t.title, t.status, t.position, '{"label":"Seed data","type":"import"}'::jsonb
from (values
  ('Plan spring content calendar',  'completed',   1),
  ('Shoot lifestyle photography',   'completed',   2),
  ('Draft six feature articles',    'completed',   3),
  ('Edit & proof articles',         'in_progress', 4),
  ('Source product partnerships',   'todo',        5),
  ('Design landing-page modules',   'todo',        6),
  ('Schedule social teasers',       'todo',        7),
  ('Newsletter announcement',       'todo',        8)
) as t(title, status, position);
